...
# Removed run-as as it only works on debuggable builds
adb shell "mkdir -p /data/data/com.termux/files/home/storage" 2>$null
adb shell "ln -sf /storage/emulated/0 /data/data/com.termux/files/home/storage/shared" 2>$null
...
if ($shizukuCheck -match "package:") {
    ...
    # Pull APK to PC, extract there, then push binaries back (more reliable than shell unzip)
    Write-Host "  .. Extracting APK on PC..." -ForegroundColor Blue
    adb pull $apkPath "$env:TEMP\shizuku.apk"

    # Use a temporary directory to unzip the APK (APKs are ZIPs)
    $extractPath = "$env:TEMP\shizuku_extracted"
    if (Test-Path $extractPath) { Remove-Item -Recurse -Force $extractPath }
    New-Item -ItemType Directory -Path $extractPath | Out-Null

    # Unzip APK using PowerShell (treat as .zip)
    Expand-Archive -Path "$env:TEMP\shizuku.apk" -DestinationPath $extractPath -Force

    # Push the specific rish binaries to the phone
    adb push "$extractPath\assets\rish" /sdcard/Shizuku/rish
    adb push "$extractPath\assets\rish_shizuku.dex" /sdcard/Shizuku/rish_shizuku.dex

    Write-Host "  OK Shizuku rish binaries pushed to /sdcard/Shizuku/" -ForegroundColor Green
} else {
...
$bootstrapLines = @(
    ...
    '# Auto-bootstrap for OpenClaude Offline',
    ...
    '',
    '# RAM Optimization for 4GB devices',
